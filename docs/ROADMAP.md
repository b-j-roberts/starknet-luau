...
Initialize the repository with modern Roblox development tooling, CI/CD, and project structure.

### 0.1 Repository Initialization

**Description**: Create the base repository structure with all configuration files, toolchain setup, and project scaffolding.

**Reference**: See `roblox-adv-testing/` root for the overall file layout, `README.md` structure (installation via Wally, prerequisites, commands, project structure sections), `LICENSE` (MIT), and `.gitignore` (ignores `*.rbxl`, `*.rbxm`, `Packages/`, `ServerPackages/`, `DevPackages/`, `sourcemap.json`, `.DS_Store`, `*.rbxl.lock`).
- Template path: `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/`
- Key files: `README.md`, `LICENSE`, `.gitignore`

**Requirements**:
- [ ] Initialize git repository
- [ ] Create `README.md` with project description, installation instructions, quick start, and API overview
- [ ] Create `CLAUDE.md` with project-specific development instructions
- [ ] Create `LICENSE` (MIT)
- [ ] Create `.gitignore` (Roblox place/model files, Packages/, sourcemap.json, .DS_Store, lock files)

---

### 0.2 Toolchain Configuration

**Description**: Configure Rokit for managing all required development tools at pinned versions.

**Reference**: See `roblox-adv-testing/rokit.toml` for the exact format and tool pinning pattern. It pins rojo, wally, lune, selene, stylua, and wally-package-types with exact versions using the `[tools]` section and `scope/tool@version` format.
- Template path: `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/rokit.toml`

**Requirements**:
- [ ] Create `rokit.toml` with pinned tool versions:
  - rojo v7.6.1
  - wally v0.3.2
  - lune v0.10.4
  - selene v0.30.0
  - stylua v2.3.1
  - wally-package-types v1.6.2

---

### 0.3 Package Management (Wally)

**Description**: Configure Wally package manifest for publishing starknet-luau as a shared-realm package.

**Reference**: See `roblox-adv-testing/wally.toml` for the exact `[package]` section format including `name`, `version`, `registry`, `realm` fields. The template uses `realm = "shared"` and points to the UpliftGames wally-index. Also see `wally.lock` for the lock file format (auto-generated by `wally install`).
- Template path: `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/wally.toml`
- Template path: `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/wally.lock`

**Requirements**:
- [ ] Create `wally.toml` with package metadata:
  - Name: `brandonroberts/starknet-luau`
  - Version: `0.1.0`
  - Registry: `https://github.com/UpliftGames/wally-index`
  - Realm: `shared`
  - Description, license, authors, include/exclude
- [ ] Add dependency: `evaera/promise@^3.1.0`
- [ ] Document peer dependency on `daily3014/cryptography@^3.1.0` (optional)
- [ ] Run `wally install` to generate lock file and Packages/

---

### 0.4 Rojo Project Configuration

**Description**: Configure Rojo to map the filesystem structure to the Roblox DataModel for both library usage and development.

**Reference**: See `roblox-adv-testing/default.project.json` for the exact Rojo project format. It maps `src/shared/` -> `ReplicatedStorage`, `src/server/` -> `ServerScriptService`, `src/client/` -> `StarterPlayer.StarterPlayerScripts`, and includes Workspace/Lighting configuration. Adapt the tree structure for a library layout (src/ -> ReplicatedStorage.StarknetLuau).
- Template path: `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/default.project.json`

**Requirements**:
- [ ] Create `default.project.json` mapping:
  - `src/` -> ReplicatedStorage.StarknetLuau (library source)
  - `Packages/` -> ReplicatedStorage.Packages
- [ ] Create `dev.project.json` for development/testing that includes examples and test scripts
- [ ] Verify `rojo build` produces a valid `.rbxm` model file

---

### 0.5 Code Quality Configuration

**Description**: Set up linting, formatting, and LSP configuration for strict Luau development.

**Reference**: See the three config files in `roblox-adv-testing/` for exact format and settings:
- `selene.toml`: Uses `std = "roblox"` (minimal config, Roblox linter defaults)
- `.stylua.toml`: Sets `syntax = "Luau"`, `column_width = 120`, `indent_type = "Tabs"`, `indent_width = 4`, `quote_style = "AutoPreferDouble"`
- `.luaurc`: Sets `"languageMode": "strict"`, `"lint": {"*": true}`, and aliases for `"Packages"` and `"ServerPackages"` pointing to their directories
- Template paths:
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/selene.toml`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/.stylua.toml`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/.luaurc`

**Requirements**:
- [ ] Create `selene.toml` with Roblox standard library
- [ ] Create `.stylua.toml`:
  - Syntax: Luau
  - Column width: 120
  - Indent: Tabs (4 width)
...
**Description**: Create a Makefile with all common development commands.

**Reference**: See `roblox-adv-testing/Makefile` for the exact target format and commands. It defines: `test` (`lune run tests/run`), `serve` (`rojo serve`), `lint` (`selene src/`), `fmt` (`stylua src/`), `install` (`wally install` + `rojo sourcemap default.project.json --output sourcemap.json` + `mkdir -p Packages` + `wally-package-types --sourcemap sourcemap.json Packages/`), and `check` (`make lint && stylua --check src/ && make test`). Extend with a `build` target for our library.
- Template path: `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/Makefile`

**Requirements**:
- [ ] `make install` -- wally install + generate sourcemap + wally-package-types
- [ ] `make test` -- lune run tests/run
- [ ] `make serve` -- rojo serve
- [ ] `make build` -- rojo build -o starknet-luau.rbxm
- [ ] `make lint` -- selene src/
- [ ] `make fmt` -- stylua src/
- [ ] `make check` -- lint + fmt check + test (full CI validation locally)

---

### 0.7 Test Runner Setup

**Description**: Create a Lune-based test runner that discovers and runs `.spec.luau` files.

**Reference**: See `roblox-adv-testing/tests/run.luau` for the complete test runner implementation. It uses `@lune/process` and `@lune/fs`, implements `describe()`, `it()`, and `expect()` with matchers (`toBe`, `toEqual`, `toBeTruthy`, `toBeFalsy`, `toThrow`), auto-discovers `.spec.luau` files in the tests directory, runs each with pcall error handling, and prints a color-coded summary with pass/fail counts. Also see the sample spec files (`tests/math.spec.luau`, `tests/strings.spec.luau`, `tests/tables.spec.luau`) for the test authoring pattern.
- Template paths:
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/tests/run.luau`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/tests/math.spec.luau`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/tests/strings.spec.luau`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/tests/tables.spec.luau`

**Requirements**:
- [ ] Create `tests/run.luau` test runner with:
  - `describe(name, fn)` for test grouping
  - `it(name, fn)` for individual tests
  - `expect(value)` with matchers: `.toBe()`, `.toEqual()`, `.toBeTruthy()`, `.toBeFalsy()`, `.toThrow()`
  - Auto-discovery of `.spec.luau` files in `tests/` subdirectories
  - Color-coded pass/fail output
  - Summary with total passed/failed counts
  - Exit with non-zero code on failure
- [ ] Create placeholder `tests/crypto/`, `tests/signer/`, `tests/provider/`, `tests/tx/`, `tests/wallet/`, `tests/contract/` directories
- [ ] Create `tests/fixtures/` directory with empty `test-vectors.luau`

---

### 0.8 CI/CD Pipeline

**Description**: GitHub Actions workflows for continuous integration and release automation.

**Reference**: See the two GitHub Actions workflows in `roblox-adv-testing/.github/workflows/`:
- `ci.yml`: Triggers on push/PR to main. Defines 4 parallel jobs (build, test, lint, fmt) all running on `ubuntu-latest` with `roblox-ts/setup-rokit@v0.1.2` for tool installation. Build job runs `rojo build -o <name>.rbxl`, test runs `lune run tests/run`, lint runs `selene src/`, fmt runs `stylua --check src/`.
- `release.yml`: Triggers on version tags `v*`. Builds a `.rbxm` model, creates a GitHub Release with auto-generated notes, and publishes to Wally registry using `WALLY_AUTH_TOKEN` secret.
- Template paths:
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/.github/workflows/ci.yml`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/.github/workflows/release.yml`

**Requirements**:
- [ ] Create `.github/workflows/ci.yml`:
  - Trigger: push and PR to main
  - Jobs: build, test, lint, fmt (all using `roblox-ts/setup-rokit@v0.1.2`)
  - Build job: `rojo build -o starknet-luau.rbxm`
  - Test job: `lune run tests/run`
  - Lint job: `selene src/`
  - Fmt job: `stylua --check src/`
- [ ] Create `.github/workflows/release.yml`:
  - Trigger: version tag `v*`
  - Build `.rbxm` artifact
  - Create GitHub Release with auto-generated notes
  - Publish to Wally registry using `WALLY_AUTH_TOKEN`

---

### 0.9 Source Directory Scaffolding

**Description**: Create the initial module structure with barrel exports (init.luau files) for all planned submodules.

**Reference**: See `roblox-adv-testing/src/` for the module layout pattern. Each directory uses an `init.luau` (or `init.server.luau` / `init.client.luau` for scripts) as the entry point. For a library, use `init.luau` files that return a table of exports. See `src/shared/Hello.luau` for the basic module pattern (a ModuleScript that returns a table/function).
- Template paths:
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/src/shared/Hello.luau`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/src/server/init.server.luau`
  - `/Users/brandonroberts/workspace/Roblox/testing/roblox-adv-testing/src/client/init.client.luau`

**Requirements**:
- [ ] Create `src/init.luau` -- main entry point exporting all submodules
- [ ] Create `src/crypto/init.luau` -- crypto module barrel
- [ ] Create `src/signer/init.luau` -- signer module barrel
- [ ] Create `src/provider/init.luau` -- provider module barrel
- [ ] Create `src/tx/init.luau` -- transaction module barrel
- [ ] Create `src/wallet/init.luau` -- wallet module barrel
- [ ] Create `src/contract/init.luau` -- contract module barrel
- [ ] Create `src/contract/presets/` directory
- [ ] All init files should export empty tables initially with TODO comments
